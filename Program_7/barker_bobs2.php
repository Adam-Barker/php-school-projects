...
if (!$link)
    ...
    if (isset($choice)) {
    	...
    	echo "<td><h2>Unit Price</h2></td>";
    	echo "<td><h2># of Units</h2></td>";
    	echo "<td><h2>Summary</h2></td>";
        foreach ($choice as $value) {            
            $resultSet = mysqli_query($link, "SELECT products.ID, entertainerauthor, title, unitPrice, unitsInStock, summary
					FROM products, prodinv WHERE products.ID = prodinv.ID AND products.ID = '$value'");
            while ($row = mysqli_fetch_assoc($resultSet)) {
                echo "<tr>";
                echo "<td align><input type='checkbox' name = 'choice[]' value ='{$row['unitPrice']}'</td>";
